...
import { getAllPendingUsers, getAllUsers, approveUser, rejectUser, updateUserRole, deleteUser } from '../services/userManagement';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ currentUser, onBack }) => {
    ...
    const handleDelete = async (uid: string) => {
        if (!confirm('¿ESTÁS SEGURO? Esto eliminará los datos del usuario permanentemente (se requiere eliminar la cuenta de Firebase auth manualmente).')) return;
        try {
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {user.uid !== currentUser.uid && (
                                            <button
                                                ...
                                                className="size-8 rounded-lg flex items-center justify-center bg-white/5 hover:bg-danger/20 text-slate-500 hover:text-danger border border-white/5 hover:border-danger/30 transition-all ml-2"
