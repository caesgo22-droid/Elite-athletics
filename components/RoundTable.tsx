...
const RoundTable: React.FC<RoundTableProps> = ({ athleteId = '1' }) => {
    ...
    // Show message if no athlete data available, but still render the full UI with technical sections
    const noAthleteData = !focusAthlete;

    const finalVerdict = !noAthleteData ? messages.find(m => m.type === 'VERDICT' && m.agent === 'HEAD_COACH') : undefined;
            ...
            <div className="order-2 lg:col-start-2 lg:row-start-1 lg:row-span-2 w-full bg-[#0A0A0A] border-l border-border-subtle flex flex-col shrink-0 h-[500px] lg:h-full overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar" ref={scrollRef}>
                    ...
                    {messages.map((msg, idx) => {
                        ...
                        return (
                            <div key={idx} className={`animate-in fade-in slide-in-from-bottom-2 duration-500 relative pl-4 border-l-2 ${borderColor}`}>
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        <span className={`text-[10px] font-black uppercase tracking-widest ${config.color || 'text-slate-400'}`}>
                                            {msg.agent.replace('_', ' ')}
                                        </span>
                                    </div>
                                ...
                                <div className={`p-4 text-xs font-mono leading-relaxed text-slate-300 ${config.bg || 'bg-slate-800'} border ${config.border || 'border-slate-700'} rounded-r-lg rounded-bl-lg shadow-sm`}>
                                    {isLast && isLoading ? (
